# Pagination Directive -- Now in TYPESCRIPT!

I was working on an app that uses pagination in two different modules by copy-pasta.  The same exxact code, slapped into two controllers.  The app has a shared module and I wanted to combine the pagination into a reusable component.  The app is using Typescript, and I'm new to it, so I decided to translate the Pagination Directive into typescript as practice.  If you find wonky TS things, many apologies, I'm still learning.  Point them out, or fork and pull request!  Thanks!

In other news, I took the  prefix off of everything because I find it redundant.  Picky, I know...and yeah, bad for backwards compatibility... *sigh*

## Current Status 5/5/2016

Typescript all complete, haven't scrubbed the app's modules off this version yet, will do tomorrow.  Pagination is working, but if you add something like:

```HTML
<ul>
    <li paginate="doll in matryoshka | itemsPerPage: 10">{{doll}}
        <ul>
            <li ng-repeat="doll in doll.others"></li>
        </ul>
    </li>
</ul>
...
The inner ng-repeat is currently getting commented out, but not put back afterwards in the link function:

```HTML
<ul>
    <li paginate="doll in matryoshka | itemsPerPage: 10">{{doll}}
        <ul>
            <!-- ng-repeat="doll in doll.others" -->
        </ul>
    </li>
</ul>
...
This seems to be an error in my typescript translation.  I modified the directives to make them classes, so it's possible the link function isn't getting called at the right time anymore.  Am continuing to search for and fix the cause of this.  It functions as expected in the [original implementaion.](http://plnkr.co/edit/I0KWzX5lRR9CblwfSMTl?p=preview)

# Original README

With minor text updates to remove dir- prefix.
...
```HTML
<ul>
    <li paginate="item in items | itemsPerPage: 10">{{ item }}</li>
</ul>
...
Alternatively just download the files `pagination.js` and `pagination.tpl.html`. Using Bower or npm has the advantage of making version management easier.
...
```JavaScript
// in your app
angular.module('myApp', ['angularUtils.directives.pagination']);
```

Then create the paginated content:
```HTML
<ANY
    paginate="expression | itemsPerPage: (int|expression) [: paginationId (string literal)]"
    [current-page=""]
    [pagination-id=""]
    [total-items=""]>
    ...
...
pagination.tpl.html file (which conforms to Bootstrap's pagination markup). Therefore, it is not necessary to specify a template.
...
If you plan to use a custom template, take a look at the default as demonstrated in pagination.tpl.html to get
...
myApp.config(function(paginationTemplateProvider) {
    paginationTemplateProvider.setPath('path/to/p
agination.tpl.html');
});
```
...
```HTML
<pagination-controls template-url="path/to/pagination.tpl.html"></pagination-controls>
```
...
* **`pagination-id`** (optional) Used to group together the paginate directive with a corresponding pagination-controls when you need more than
...
i.e. `<pagination-controls on-page-change="myMethod(newPageNumber, oldPageNumber)">`, and the method you specify must be defined on your controller $scope.

* **`pagination-id`** (optional) Used to group together the pagination-controls with a corresponding paginate when you need more than
one pagination instance per page. See the section below on setting up multiple instances.

* **`template-url`** (optional, default = `directives/pagination/pagination.tpl.html`) Specifies the template to use.

* **`auto-hide`** (optional, default = true) Specify whether the pagination-controls should be hidden when there's not enough elements to paginate over.

Note: you cannot use the `pagination-controls` directive without `paginate`. Attempting to do so will result in an
exception.

## Writing A Custom Pagination-Controls Template

The default template ([pagination.tpl.html](pagination.tpl.html)) is based on the [Bootstrap pagination markup](http://getbootstrap.com/components/#pagination). If you wish to modify the template or write your own,
...
As with the [ngRepeat directive](https://docs.angularjs.org/api/ng/directive/ngRepeat#special-repeat-start-and-end-points), you can use the `-start` and `-end` suffix on the `paginate` directive to
repeat a series of elements instead of just one parent element:

```HTML
<header paginate-start="item in items | itemsPerPage: 10">
    Header {{ item }}
</header>
<div class="body">
  Body {{ item }}
...
1. Specify the `pagination-id` attribute on the `paginate` directive.
3. Specify the `pagination-id` attribute on the `paginations-controls` directive.
...
<ul>
    <li paginate="customer in customers | itemsPerPage: 10" pagination-id="cust">{{ customer.name }}</li>
</ul>

<pagination-controls pagination-id="cust"></pagination-controls>

<!-- second pagination instance -->
<ul>
    <li paginate="branch in branches | itemsPerPage: 10" pagination-id="branch">{{ customer.name }}</li>
</ul>

<pagination-controls pagination-id="branch"></pagination-controls>
```
...
<div ng-repeat="list in lists">
    <ul>
        <li paginate="item in list.collection | itemsPerPage: 3" pagination-id="list.id">ID: {{ list.id }}, item: {{ item }}</li>
    </ul>
    <pagination-controls pagination-id="list.id"></pagination-controls>
</div>
```
...
The solution is to use the `total-items` attribute on the `paginate` directive. Commonly, in such a server-side paging scenario, the server would also return some meta-data
...
<div ng-controller="UsersController">
    <table>
        <tr paginate="user in users | itemsPerPage: usersPerPage" total-items="totalUsers" current-page="pagination.current">
            <td>{{ user.name }}</td>
            <td>{{ user.email }}</td>
        </tr>
    </table>

    <pagination-controls on-page-change="pageChanged(newPageNumber)"></pagination-controls>
</div>
````
...
```HTML
<li paginate="item in collection | itemsPerPage: 10 | filter: q">...</li> <!-- BAD -->
```
...
```HTML
<li paginate="item in collection | filter: q | itemsPerPage: 10">...</li> <!-- GOOD -->
```

### What is the `paginationService` and why is it not documented?

The [`paginationService`](https://github.com/michaelbromley/angularUtils/blob/6055d260be44c0ba221a8c9bea015ac97e836a10/src/directives/pagination/pagination.js#L466-L521) is used internally to facilitate communication between the instances of the `pagination` and `pagination-controls` directives. Due to the way Angular's dependency injection system works, the service will be exposed in your app, meaning you can inject it directly into your controllers etc.
...
writing unit tests to cover your change(s). All unit tests are contained in the `pagination.spec.js` file, and
